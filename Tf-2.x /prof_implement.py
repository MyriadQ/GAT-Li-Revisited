...
class GATMILModel(Model):
    ...
    def train_step(self, data, training_mask=False):
        ...
        with tf.GradientTape() as tape:
            ...
            # Compute loss - for mask training, only use log loss (matches TF1 loss_explainer)
            if training_mask:
                # Phase 2: Only log loss (matches TF1 loss_explainer)
                loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(y_batch, y_pred))
            else:
                # Phase 1: Total loss with L2 regularization
                loss, _, _ = self.compute_loss(y_batch, y_pred)
            # Compute accuracy
